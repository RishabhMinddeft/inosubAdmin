  ...
  const upDateHash = async () => {
    ...
    try {
      await nftContractInstance.methods
        .updateProject(blockChainId, merkleHash)
        ...
        <USHOuter>
          ...
          {process === 1 && (
            <CWBtn
              onClick={() => {
                console.log("this");
                fetchSnapshotWinnersData(selectedProjectId);
              }}
            >
              <MdOutlineFindReplace /> Fetch Snapshot Selected User Data
            </CWBtn>
          )}
          ...
          {process === 2 && (
            <>
              ...
              <GBtn
                disabled={merkleHash ? true : false}
                onClick={() => generateMerkleHash()}
              >
...
const mapDipatchToProps = (dispatch) => {
  return {
    ...
    addMerkleHash: (projectId, merkleHash) =>
      dispatch(actions.addMerkleHash(projectId, merkleHash)),
